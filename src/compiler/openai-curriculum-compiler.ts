...
For every concept, include at least one independently reviewed keypad transfer question with two to four short spoken choices, stable answer IDs, exactly one correct answer, and a featurePhoneSms under 100 characters. Keypad items must test the reviewed objective without relying on visual notation and must not by themselves establish secure mastery.
For every concept, include safe household anchor activities with generic object names, offline learner signals, one original response lead, and one question grounded in the reviewed learning objective. Do not require purchase, ingestion, heat, electricity, sharp tools, chemicals, or unsupervised risk.
