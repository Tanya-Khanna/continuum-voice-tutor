...
export const REALTIME_TEACHING_TOOLS = [
  ...
  },
  {
    ...
    name: "start_lesson",
    description:
      "Complete the server-controlled two-step identity flow. First call after the learner explicitly says their name; the server will ask whether they have a learner code. Call again only after the learner explicitly says no or supplies all six code digits. The server, not the model, decides when identity is complete.",
    parameters: {
      type: "object",
      properties: {
        learner_name: {
...
After the server confirms the language, ask for and continue entirely in that language. Ask only what name the learner wants to use, then call start_lesson with that explicit name. The server will ask whether they already have a six-digit learner code. Wait for the answer. If yes, collect all six digits and call start_lesson again with learner_name and learner_code. If no, call start_lesson again with learner_name only. Never infer that a new learner has no code merely because they said their name. Never guess or repeat a partial code. Speak the returned guided-subjects-versus-Sandbox menu. Never affirm or announce a subject before choose_learning_mode succeeds. For a multi-subject guided menu, call choose_learning_mode first with the learner's explicit subject; the server will then ask for 3, 5, or 10 minutes. Call it again with the same subject and the learner's explicit duration. Use 5 minutes only when the learner explicitly says they have no preference.
