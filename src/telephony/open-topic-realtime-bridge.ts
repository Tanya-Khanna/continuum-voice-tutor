...
const CallbackReminderProposalArgumentsSchema = z.object({
  source_text: z.string().trim().min(1).max(2_000),
  topic: z.string().trim().min(1).max(120),
  due_at: z.string().datetime(),
  time_zone: z.string().trim().min(1).max(80),
});
const ConfirmSmsReminderArgumentsSchema = z.object({
...
export interface ConfirmedSmsReminder {
  callerNumber: string;
  learner: LearnerProfile;
  kind: "exam_review" | "callback_nudge";
  topic: string;
  dueAt: string;
  examAt: string | null;
}
...
function speakToolOutputEvent(
  ...
  const languageContract = language
    ? ` Speak only in ${language.displayName} (${language.languageMode}). Do not fall back to English.`
    : "";
  const instructions =
    policy === "exact"
      ? `The latest function output is authoritative. Speak its spoken_response exactly, add nothing, then wait.${languageContract}`
...
function respondToTranscriptEvent(
  ...
  const languageContract = language
    ? ` The learner selected ${language.displayName} (${language.languageMode}). Speak only in ${language.displayName}; do not fall back to English.`
    : "";
  const stageContract =
    stage === "identity"
      ? " Call the identity tool immediately. Output only the function call: produce no audio or conversational text before the tool result."
      : "";
  return {
    type: "response.create",
    response: {
      instructions: `The server verified this transcript for the current ${stage} stage: ${JSON.stringify(transcript)}. Treat only these words as learner input. Use the current tool and do not infer any unstated choice.${stageContract}${languageContract}`,
    },
  };
}
...
export class OpenTopicRealtimeController {
  ...
  readonly #onLessonPaused:
    ...
    | undefined;
  readonly #onSmsReminderConfirmed:
    | ((reminder: ConfirmedSmsReminder) => Promise<void> | void)
    | undefined;
  ...
  constructor(options: {
    ...
    onSmsReminderConfirmed?: (
      reminder: ConfirmedSmsReminder,
    ) => Promise<void> | void;
  }) {
    ...
    this.#onSmsReminderConfirmed = options.onSmsReminderConfirmed;
  }
    ...
    try {
      ...
      if (call.name === "select_language") {
        ...
        if (!this.#selectedLanguage) {
          ...
          if (args.source_text !== transcript) {
            output = {
              ok: false,
              state_changed: false,
      ...
      } else if (call.name === "propose_callback_reminder") {
        const args = CallbackReminderProposalArgumentsSchema.parse(
          JSON.parse(call.arguments),
        );
        const transcript = this.#lastVerifiedTranscript;
        const dueAt = new Date(args.due_at);
        if (!this.#learner || !this.#context) {
          output = {
            ...
            state_changed: false,
            spoken_response:
              "Before setting a reminder, what name would you like me to use?",
          };
          nextStage = "identity";
        } else if (!transcript || args.source_text !== transcript) {
          output = {
          ...
          dueAt <= new Date() ||
          dueAt.getTime() - Date.now() > 365 * 24 * 60 * 60_000
        ) {
          output = {
            ok: false,
            state_changed: false,
            spoken_response:
              "I need a future callback reminder time within one year. What date and time should I use?",
          };
        } else {
          const displayTime = formattedReminderTime(
            args.due_at,
          ...
          output = {
            ok: true,
            reminder_pending_consent: true,
            spoken_response: `One SMS reminder to call Continuum about ${args.topic} at ${displayTime}. Should I schedule it? Say yes or no, or press 1 or 2.`,
          };
          policy = "localize_onboarding";
        }
      } else if (call.name === "confirm_sms_reminder") {
        const args = ConfirmSmsReminderArgumentsSchema.parse(
          JSON.parse(call.arguments),
        );
  ...
  async #applyExamReminderConsent(consentConfirmed: boolean): Promise<string> {
    ...
    if (!this.#onSmsReminderConfirmed) {
      return `SMS reminders are not enabled for this call. ${nextPrompt}`;
    }
    try {
      await this.#onSmsReminderConfirmed({
        callerNumber: this.#callerNumber,
    ...
    if (this.#dynamicToolRouting) {
      send(
        buildOpenTopicToolUpdate(
          ...
          this.#selectedLanguage
            ? {
                languageMode: this.#selectedLanguage.languageMode,
  ...
  constructor(options: {
    ...
    this.#controller = new OpenTopicRealtimeController({
      ...
      ...(options.onLessonPaused
        ...
        : {}),
      ...(options.onSmsReminderConfirmed
        ? { onSmsReminderConfirmed: options.onSmsReminderConfirmed }
        : {}),
    });
  }
