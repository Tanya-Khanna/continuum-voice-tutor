...
const DashboardTurnSchema = z.object({
  ...
  diagnosis: z.string(),
  diagnosis_basis: z.string().nullable(),
  misconception: z.string().nullable(),
  trusted_phase: z.string().nullable(),
  transition_authority: z.string().nullable(),
  policy_checks: z.array(z.string()),
  ...
  const sessions = options.repository
    ...
    .map((session) => {
      ...
      const guidedTurns = options.repository.listTurns(session.id).map((entry, index) => ({
        ...
        diagnosis_basis: decisions[index]?.diagnosisBasis ?? null,
        misconception: decisions[index]?.misconception ?? null,
        trusted_phase: decisions[index]?.trustedPhase ?? null,
        transition_authority: decisions[index]?.transitionAuthority ?? null,
        policy_checks: decisions[index]?.policyChecks ?? [],
