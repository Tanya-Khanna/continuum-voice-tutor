...
export function renderLandingPage(options: {
  ...
  const subjects = options.guidedSubjects.map(escapeHtml).join(" · ");
  const cta = callable
...
<section class="hero"><div><p class="eyebrow">The call is the classroom</p><h1>Learning,<br>without bars.</h1><p class="lead">A personalized tutor that calls you, remembers what helps you learn, and teaches in your language—using an ordinary phone and tiny SMS messages.</p>${cta}<p class="fine">No smartphone. No app. No camera. No internet or mobile data. Calls are sponsored only where the local deployment says so.</p></div>
<div class="phone" aria-label="Illustration of a basic keypad phone"><div class="screen"><div class="bars"><i></i><i></i><i></i><i></i></div>Lesson paused at Q2.<br>Call back anytime.</div><div class="keys">${"<i></i>".repeat(12)}</div></div></section>
<section class="grid"><article class="card"><b>It teaches</b>Diagnoses the misconception, changes method when an explanation fails, asks for teach-back, and checks transfer.</article><article class="card"><b>It remembers safely</b>Progress, helpful methods, homework, and approved goals—not raw recordings or unnecessary personal stories.</article><article class="card"><b>It comes back</b>Consented schedules, retrieval calls, missed-lesson reminders, and short homework by SMS.</article></section>
...
<section class="proof"><p class="eyebrow">Reviewed guided menu</p><p>${subjects || "No curriculum pack is public yet."}</p><p class="fine">Ask Anything supports broader safe curiosity but never awards formal curriculum mastery. Public subjects appear only after their source, review, compilation, and release gates pass.</p></section>
