...
- Pushed implementation commit `6d73e6c`; GitHub's release gate passed. Railway deployment `ff752795-c420-4589-850c-80032f65a22a` succeeded and public health reports the exact `6d73e6cd54ba` revision, OpenAI teaching, Realtime configured, Math-only reviewed catalog, all four access switches enabled, and the public phone unpublished.
- The independent clean-clone gate installed from the lockfile, built and smoke-tested production, passed 181/181 tests and 25/25 deterministic eval cases, seeded a paused lesson, and resumed the exact pending question with no local secrets or previous state.
- The secret-free release preflight reached 6/8: committed revision, clean worktree, 11/11 phone configuration, production health, 4/4 access switches, and exact deployed revision all pass. The only two holds are the four genuine human curriculum approvals and the final human carrier/submission acceptance receipt.
- Replaced the unreliable ePathshala wrapper URLs in all four pending source briefs, the release guide, and README with direct official NCERT chapter PDFs after the wrapper pages timed out. The changed source URL sets deliberately invalidate any downstream approval digest; no approval or frozen pack existed yet.

## 2026-07-20 — Four model-verified curriculum candidates

- Recorded Tanya Khanna's explicit human review of the four direct official NCERT source chapters as create-only source-approval receipts. This authorizes compilation scope only; it does not claim that the generated teaching content has been spot-checked or released.
- Hardened the GPT-5.6 Terra compiler for OpenAI Structured Outputs: unsupported generated regex patterns are removed only from the model-facing schema while the full local Zod validators remain authoritative; verifier-wide issues use required nullable concept IDs; placement recommendations now pass the same integrity checks during bounded correction.
- Added trusted exact-source checks for deployment metadata, concept IDs, and required vocabulary. Approved vocabulary terms and definitions are preserved deterministically while model-authored bridge copy remains independently verified.
- Added a bounded compiler ↔ independent-verifier correction loop. It caught and repaired source-like wording, material/property confusion, ambiguous teaching signals, repeated examples instead of transfer, incomplete SMS choice sets, and inconsistent reference-point instructions before candidates were written.
- Compiled Science, English, History, and Geography into digest-bound candidates, then performed a second code-assisted teaching spot-check. Corrections to the English object activity, History ordering prompt, Science misconception/transfer item, and Geography SMS/transfer item were independently re-verified with clean receipts.
...
## 2026-07-20 — Comprehensive release audit

- Verified the exact deployed revision `1c40fb9234e2991edff83406d194cc1365643715`: the Git worktree and upstream `main` match, GitHub's Release gate is green, Railway reports the same revision, all five reviewed subjects, OpenAI teaching, Realtime configuration, and all four callback/SMS/scheduler/recap feature switches.
- Passed strict TypeScript, 185/185 automated tests, 25/25 deterministic teaching cases, 24/24 live agent scenarios, the five-pack digest-bound release gate, 11/11 phone configuration, the compiled production smoke, and a fresh-clone install/build/test/eval/seed/exact-resume journey. Direct offline turns were also rendered successfully from Math, Science, English, History, and Geography.
- Passed the live GPT-5.6 checks for Hindi-English, Spanish-English, French-English, history summarization, safety/prompt-injection handling, and Spanish placement. Ask Anything passed on retry but one of two live runs violated the exact contract by returning `es-CO+en` instead of the test's `es+en` and more than one question; treat that as a nondeterministic output-contract issue, not a fully green live gate.
- `npm run smoke:realtime` failed twice because its synthetic input provides only a name while the current Realtime instructions require the learner-code answer before `start_lesson`. A corrected live text-only handshake containing both the name and “no learner code” produced the expected `start_lesson` function call. The API and production prompt path are reachable, but the repository smoke command is stale and must be corrected before release.
- Production security checks passed: protected readiness/session/metric endpoints return `401` without the dashboard token and `200` with it; forged Twilio missed-call, SMS, call-status, and message-status webhooks return `403`; an unsigned OpenAI webhook returns `400`; ranged sample audio returns `206`; Railway emitted no recent error or warning logs.
- Desktop and mobile landing pages rendered correctly. Mission Control tabs, Eval gate, Sample, Outcomes, Release, and transcript/audio selection are interactive; the five-tab mobile navigation overflows horizontally at 390 px, and the sample transcript still labels tutor lines `nomad` instead of `Continuum`.
- Final release preflight remains honestly at 7/8. The complete real-carrier matrix, five consecutive golden judge journeys, private acceptance receipt, video/YouTube URL, `/feedback` Session ID, Devpost team acceptance, repository-sharing confirmation, dashboard judge URL, and human-authored submission copy are not machine-verifiable and remain open. `NOMAD_PUBLIC_PHONE_ENABLED` correctly remains false.
- Locked-scope reminder: missed-call callback, DTMF, silence recovery, portable identity, cross-phone resume, scheduling, guardian controls, Curiosity Trails, and five subjects are implemented. WhatsApp, camera homework, and GSAP/sound were explicitly excluded by the active v6 plan and do not exist to test.

## 2026-07-21 — Genuine Twilio webhook signature fix

- Tanya's first missed-call acceptance attempt reached Twilio twice, but Twilio recorded error `11200` because production returned `403 invalid_twilio_signature`. Both calls ended at zero seconds and the production callback ledger remained empty, proving the job was rejected before any outbound callback or OpenAI SIP bridge.
- The configured Twilio account/token, public webhook URL, HTTP method, caller format, country allowlist, adult-demo switch, and Railway variables matched. A locally generated signed probe passed the same production boundary, narrowing the defect to a genuine-provider signature variant rather than missing credentials or stripped headers.
- Root cause: Continuum's custom HMAC validator checked only the URL without an explicit standard port. Twilio's official Node validator deliberately checks both variants because the provider's signing backend can include `:443` or `:80` inconsistently. Replaced the manual comparison with Twilio's official validator while retaining the local signature generator for deterministic fixtures.
- Added a regression where a signature generated for `https://…:443` must validate against the configured portless HTTPS URL. Focused callback/SMS/carrier tests, strict TypeScript, all 185 automated tests, the 25/25 deterministic eval, five-pack release gate, 11/11 phone preflight, and the production build pass before deployment.
- Reduced missed-call duplicate collapse from ten minutes to one minute at Tanya's request. Rapid repeated rings remain deduplicated, while a learner can retry after a failed or missed callback without an excessive wait.

## 2026-07-21 — Language-first, silence-safe phone onboarding

- Reproduced Tanya's carrier observation: the learner heard English identity questions before choosing a language, and the Realtime model could announce Science after silence/noise because server VAD automatically created a response and the application trusted the model's subject argument.
- Moved language selection to the first call state. The India flagship entrypoint now offers a deterministic 1–9 menu for English, Hindi, Spanish, French, Kiswahili, Tamil, Bengali, Arabic, and Urdu, with a `*` path for another spoken BCP-47 language. This finite menu is injected deployment configuration, not a language list in the teaching, persistence, curriculum, or telephony domain.
- Configured Realtime input transcription and disabled VAD's automatic response creation. Trusted code now creates the model response only after receiving a meaningful transcript; blank transcription and transcription failure repeat the current prompt without changing state.
...
## 2026-07-21 — Open-topic production vertical slice

- Added the versioned open-topic domain contract: trusted phases, `LearningIntent`, `TopicPlan`, knowledge state, structured activity, evidence, keypad choices, optional SMS follow-up, and human-support decision. OpenAI Responses uses Zod Structured Outputs; the local engine preserves the same state-machine boundary without paid API calls.
- Added a pack-free `OpenTopicLessonService` and runtime. It persists the exact next question before speech, records objective-specific evidence and pedagogy decisions, caps DTMF-only understanding at developing, requires meaningful method changes after not-helpful feedback, stores only consented preferences, and redacts contact/location data before model input or persistence.
- Replaced the production Realtime call route with a language → identity → open-topic sideband controller. It offers no subject, grade, mode, curriculum, or duration menu. Verified server transcripts—not model tool arguments—drive teaching state; blank audio cannot advance state.
- Added open-topic SIP/DTMF behavior for language, portable identity, repeat, hint, keypad fallback, answers, and learner feedback. Deterministic integration tests prove method switching and exact cross-phone resume with the portable learner code.
- Changed `npm run chat` to the open-topic teacher and removed curriculum loading from that zero-credit learner journey. Historical curriculum code and tests remain immutable fixtures, not the production call runtime.
- Updated Mission Control to display open-topic sessions without requiring a curriculum pack. The redacted proof now exposes activity, diagnosis, reasoning evidence, strategy change, evidence kind/result, knowledge boundary, human-support route, and model usage.
...
## 2026-07-21 — v7 release proof, safe migration, and complete SMS thread

- Hardened the structured diagnosis boundary: a topic request cannot become a misconception, every diagnosis names its evidence basis, non-stable knowledge cannot receive secure mastery, and medical, abuse, immediate-danger, prompt-injection, and disputed/current signals fail closed through trusted policy.
- Added an application-owned pedagogy trace for verified learner words, trusted phase/activity and phase/evidence transitions, supported diagnosis, failed-method handling, exact pending question, mastery cap, knowledge boundary, and voice policy. Mission Control exposes the redacted trace without hidden chain-of-thought.
- Added a one-time, marker-backed legacy migration. Compatible historical summaries are PII-redacted into private legacy learning memory, old rows remain intact for audit, and imported mastery can guide a recall question but can never become current secure evidence.
- Completed the bounded SMS relationship thread: normal lesson recap, one-question practice/reply, pause notice, separately consented exam/revision reminder, callback nudge, guardian progress/memory controls, quiet hours, STOP cancellation, one-segment rendering, signed/idempotent webhooks, and exact-phone authorization. Recurring lesson calls remain disabled.
- Replaced the stale curriculum Realtime smoke and release receipts with language → identity → open-topic flows. Production checks now require the pack-free v7 experience, callback/SMS/reminder/recap flags, and the recurring scheduler disabled.
- Added a nine-case spend-guarded live GPT v7 evaluation for arbitrary English content, Hinglish, Spanish, method switching, prompt injection, medical and immediate-safety boundaries, current/disputed facts, and DTMF mastery. Reports are schema-validated, tied to the exact Git revision, displayed in Mission Control, and required by release preflight; the suite is implemented but deliberately not run without `--confirm-spend` on the final commit.
- Current zero-credit verification: strict TypeScript, 228/228 automated tests, 39/39 deterministic v7 anti-wrapper cases, production build, and pack-free production smoke all pass.
- The first committed clean-archive run exposed that the old verifier seeded a legacy Math session and only assumed v7 resume. Replaced the demo seed with a real open-topic checkpoint and made the gate capture and assert `Session: resumed`, the exact pending prompt, and the unchanged turn count. A second clean archive then installed from the lockfile, ran 228/228 tests and 39/39 evals, built/smoke-tested the pack-free server, and visibly resumed the exact open-topic question.
- The first nine-case live GPT v7 run passed six cases and correctly blocked three inconsistent safety outputs. GPT had occasionally attached a misconception to an initial disclosure, treated a medical request as both high-stakes and unsafe without aligning the state, or selected a safety state without its required human-support route. Moved those non-negotiable rules into `applyTrustedOpenTopicInvariants`: the application now clears invented initial evidence, derives a conservative knowledge boundary from structured flags, and deterministically enforces the human-support route before policy validation. Tightened the prompt so control injection is not itself mislabeled as harmful content. All three formerly failing live cases pass targeted reruns.
- The next full live run passed eight of nine. The remaining current-election case correctly selected `current_or_disputed` and withheld mastery, but did not explicitly choose the structured `uncertainty` method. Trusted policy now requires that method for every current/disputed state, giving the correction loop a language-independent contract instead of relying on keyword matching in spoken prose. The formerly failing case passes its targeted live rerun.
- A later full run exposed one nondeterministic two-question English opening. The lesson service would have rejected it, but only after the Responses engine exhausted its normal path, risking a caller-facing error. Added a reusable open-topic voice policy to the model retry loop: exactly one question, at most three sentences, no markup/URL, no symbolic fractions, and the exact saved final question. The formerly failing live case passes its targeted rerun.
